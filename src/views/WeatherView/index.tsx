    ...
    return <>
        <div className="h-screen">
            <div className={isMobile ? "flex-col h-1/2" : `flex h-1/4`}>
                {/* Section A (25% width) */}
                <div className={isMobile ? "pt-1 pl-1 pr-1 pb-0.5 h-1/2" : "w-1/4 pt-1 pl-1 pr-0.5 pb-0.5"}>
                    {/* Content for Section A */}
                    {(currentWeather && currentUnits) &&
                        <CurrentWeather currentWeather={currentWeather} currentUnits={currentUnits}/>}
                </div>

                {/* Section B (75% width) */}
                <div className={isMobile ? "pt-0.5 pl-1 pr-1 pb-0.5 h-1/2" : "w-3/4 pl-0.5 pt-1 pr-1 pb-0.5"}>
                    {/* Content for Section B */}
                    {(dailyWeather && dailyUnits) &&
                        <DailyWeather dailyWeather={dailyWeather} dailyUnits={dailyUnits}/>}
                </div>
            </div>
            <div className={isMobile ? "pt-0.5 pr-1 pl-1 pb-1 h-1/2" : "w-full h-3/4 pr-1 pl-1 pb-1 pt-0.5"}>
